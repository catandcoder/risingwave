...
impl NexmarkConfig {
    pub fn from(properties: &Properties) -> anyhow::Result<Self> {
        let active_people =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_ACTIVE_PEOPLE, 1000)?;
        let in_flight_auctions =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_IN_FLIGHT_AUCTIONS, 100)?;
        let out_of_order_group_size =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_OUT_OF_ORDER_GROUP_SIZE, 1)?;
        let hot_seller_ratio =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_HOT_SELLER_RATIO, 4)?;
        let hot_auction_ratio =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_HOT_AUCTION_RATIO, 2)?;
        let hot_bidder_ratio =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_HOT_BIDDER_RATIO, 4)?;
        let first_event_id =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_FIRST_EVENT_ID, 0)?;
        let first_event_number =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_FIRST_EVENT_NUMBER, 0)?;
        let num_categories =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_NUM_CATEGORIES, 5)?;
        let auction_id_lead =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_AUCTION_ID_LEAD, 10)?;
        let hot_seller_ratio_2 =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_HOT_SELLER_RATIO_2, 100)?;
        let hot_auction_ratio_2 =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_HOT_AUCTION_RATIO_2, 100)?;
        let hot_bidder_ratio_2 =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_HOT_BIDDER_RATIO_2, 100)?;
        let person_proportion =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_PERSON_PROPORTION, 1)?;
        let auction_proportion =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_AUCTION_PROPORTION, 3)?;
        let bid_proportion =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_BID_PROPORTION, 46)?;
        let proportion_denominator = person_proportion + auction_proportion + bid_proportion;
        let first_auction_id =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_FIRST_AUCTION_ID, 1000)?;
        let first_person_id =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_FIRST_PERSON_ID, 1000)?;
        let first_category_id =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_FIRST_CATEGORY_ID, 10)?;
        let person_id_lead =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_PERSON_ID_LEAD, 10)?;
        let sine_approx_steps =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_SINE_APPROX_STEPS, 10)?;
        let base_time =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_BASE_TIME, NEXMARK_BASE_TIME)?;
        let us_states = split_string_arg(NexmarkConfig::get_or(
            properties,
            NEXMARK_CONFIG_US_STATES,
            "az,ca,id,or,wa,wy",
        ));
        let us_cities = split_string_arg(NexmarkConfig::get_or(
            properties,
            NEXMARK_CONFIG_US_CITIES,
            "phoenix,los angeles,san francisco,boise,portland,bend,redmond,seattle,kent,cheyenne",
        ));
        let first_names = split_string_arg(NexmarkConfig::get_or(
            properties,
            NEXMARK_CONFIG_FIRST_NAMES,
            "peter,paul,luke,john,saul,vicky,kate,julie,sarah,deiter,walter",
        ));
        let last_names = split_string_arg(NexmarkConfig::get_or(
            properties,
            NEXMARK_CONFIG_LAST_NAMES,
            "shultz,abrams,spencer,white,bartels,walton,smith,jones,noris",
        ));
        let rate_shape =
            if NexmarkConfig::get_or(properties, NEXMARK_CONFIG_RATE_SHAPE, "sine") == "sine" {
                RateShape::Sine
            } else {
                RateShape::Square
            };
        let rate_period = NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_RATE_PERIOD, 600)?;
        let first_rate = NexmarkConfig::get_as_or(
            properties,
            NEXMARK_CONFIG_FIRST_EVENT_RATE,
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_EVENTS_PER_SECOND, 10_000)?,
        )?;
        let next_rate =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_NEXT_EVENT_RATE, first_rate)?;
        let us_per_unit =
            NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_US_PER_UNIT, 1_000_000)?; // Rate is in μs
        let generators = NexmarkConfig::get_as_or(properties, NEXMARK_CONFIG_THREADS, 1)? as f32;
    ...
    fn get_as_or<T: FromStr + std::fmt::Display>(
        ...
        value
            .parse::<T>()
            .map_err(|_| anyhow!("Failed to parse value {} of key {}", value, key))
    }

    /// Returns the value for the given key or a default value if the key does
    /// not exist.
    fn get_or(properties: &Properties, key: &str, default: &str) -> String {
        properties
            .get_nexmark(key)
            .unwrap_or_else(|_| default.to_string())
    }
